...
const buttonVariants = cva(
  "inline-flex items-center justify-center gap-2 whitespace-nowrap rounded-lg text-sm font-medium ring-offset-background transition-all duration-200 ease-in-out focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2 disabled:pointer-events-none disabled:opacity-50 [&_svg]:pointer-events-none [&_svg]:size-4 [&_svg]:shrink-0",
  {
    variants: {
      variant: {
        default: "bg-primary text-primary-foreground hover:bg-primary/85 shadow-md hover:shadow-lg", // Softer hover, added shadow
        destructive:
        ...
        ghost: "hover:bg-accent/15 hover:text-accent-foreground", // Softer hover bg
        link: "text-primary underline-offset-4 hover:underline hover:text-primary/80",
      },
      size: {
        default: "h-10 px-4 py-2",
        sm: "h-9 rounded-md px-3", // Retained for consistency if specific smaller buttons are needed
        lg: "h-11 rounded-lg px-8", // Ensured lg also uses rounded-lg
        icon: "h-10 w-10 rounded-lg", // Ensure icon buttons are also rounded-lg
      },
    },
    defaultVariants: {
      variant: "default",
